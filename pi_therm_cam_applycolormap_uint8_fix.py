...
class pithermalcam:
    # See https://gitlab.com/cvejarano-oss/cmapy/-/blob/master/docs/colorize_all_examples.md
    ...
    def _setup_therm_cam(self):
        ...
        # Match the known-working configuration from `thermal_videos/pi_therm_cam.py`
    ...
    def _pull_raw_image(self):
        ...
        try:
            ...
            # Percentile clipping (stabilizes scaling a lot)
            low = np.percentile(self._raw_image, 5)
            high = np.percentile(self._raw_image, 95)
            self._raw_image = np.clip(self._raw_image, low, high)
            self._temp_min = np.min(self._raw_image)
            self._temp_max = np.max(self._raw_image)

            # Convert temps -> uint8 pixels suitable for cv2.applyColorMap
            self._raw_image = self._temps_to_rescaled_uints(self._raw_image, self._temp_min, self._temp_max)
            self._current_frame_processed = False  # newly updated raw frame has not been processed
        except ValueError:
    ...
    def _process_raw_image(self):
        ...
        # OpenCV cv2.applyColorMap requires uint8 input (CV_8UC1 or CV_8UC3).
        # ndimage.zoom produces floats, so we must clip+cast before applyColorMap.
        if self._interpolation_index == 5:  # Scale via scipy only - slowest but seems higher quality
            zoomed = ndimage.zoom(self._raw_image, 25)  # interpolate with scipy
            zoomed = np.clip(zoomed, 0, 255).astype(np.uint8)
            self._image = cv2.applyColorMap(zoomed, cmapy.cmap(self._colormap_list[self._colormap_index]))
        elif self._interpolation_index == 6:  # Scale partially via scipy and partially via cv2 - mix
            zoomed = ndimage.zoom(self._raw_image, 10)  # interpolate with scipy
            zoomed = np.clip(zoomed, 0, 255).astype(np.uint8)
            self._image = cv2.applyColorMap(zoomed, cmapy.cmap(self._colormap_list[self._colormap_index]))
            self._image = cv2.resize(self._image, (800, 600), interpolation=cv2.INTER_CUBIC)
        else:
            raw_u8 = self._raw_image
            if raw_u8.dtype != np.uint8:
                raw_u8 = np.clip(raw_u8, 0, 255).astype(np.uint8)
            self._image = cv2.applyColorMap(raw_u8, cmapy.cmap(self._colormap_list[self._colormap_index]))
            self._image = cv2.resize(
                self._image, (800, 600), interpolation=self._interpolation_list[self._interpolation_index]
            )
    ...
    def _add_image_text(self):
        ...
        if self.use_f:
            ...
            text = (
                ...
                f" - Colormap: {self._colormap_list[self._colormap_index]}"
                f" - Filtered: {self.filter_image}"
            )
        else:
            text = (
                f"Tmin={self._temp_min:+.1f}C - Tmax={self._temp_max:+.1f}C - FPS={1/(time.time() - self._t0):.1f}"
                f" - Interpolation: {self._interpolation_list_name[self._interpolation_index]}"
                f" - Colormap: {self._colormap_list[self._colormap_index]}"
                f" - Filtered: {self.filter_image}"
            )
    ...
    def _set_click_keyboard_events(self):
        ...
        if key == ord("s"):  # save snapshot
            self.save_image()
        elif key == ord("c"):  # cycle colormap forward
            self.change_colormap()
        elif key == ord("x"):  # cycle colormap backward
            self.change_colormap(forward=False)
        elif key == ord("f"):  # toggle filtering
            self.filter_image = not self.filter_image
        elif key == ord("t"):  # toggle units between C/F
            self.use_f = not self.use_f
        elif key == ord("u"):  # previous interpolation
            self.change_interpolation()
        elif key == ord("i"):  # next interpolation algorithm
    ...
    def _temps_to_rescaled_uints(self, f, Tmin, Tmax):
        """Convert temperatures to pixel intensities (uint8) suitable for cv2.applyColorMap."""
        f = np.nan_to_num(f)
        # Keep identical scaling logic to `thermal_videos/pi_therm_cam.py`.
